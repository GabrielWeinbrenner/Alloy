...
class Mouse {
    ...
    public static func GetMouseViewportPosition()->SIMD2<Float>{
        let x = (overallMousePosition.x - Preferences.ScreenSize.x * 0.5) / (Preferences.ScreenSize.x * 0.5)
        let y = (overallMousePosition.y - Preferences.ScreenSize.y * 0.5) / (Preferences.ScreenSize.y * 0.5)
